...
const isAdmin = userSnap.exists() && String(userSnap.data().role || "").toLowerCase() === "admin";
if (!isAdmin) {
  alert("غير مخول");
...
async function deactivateOtherActiveExams(activeId){
  const qy = query(collection(db,"exams"), where("status","==","active"));
  const snap = await getDocs(qy);
  const jobs = [];
  snap.forEach(d=>{
    if (d.id !== activeId) jobs.push(updateDoc(doc(db,"exams",d.id), { status:"closed" }));
  });
...
function render(){
  ...
  hint.textContent =
    `ExamID: ${examId} | Status: ${examData.status || "draft"} | Questions: ${examData.questions.length}`;
  ...
  examData.questions.forEach((q, idx)=>{
        ...
        optsBox.innerHTML = `
          ...
          <div class="row">
            ...
            <div class="field" style="min-width:260px">
              ...
              <input class="qCorrect" value="${escapeHtml(q.correctAnswer)}" placeholder="اكتب نص الخيار الصحيح">
            </div>
        ...
        optsBox.innerHTML = `
          <div class="row">
            <div class="field" style="min-width:420px">
              ...
              <input class="qCorrect" value="${escapeHtml(q.correctAnswer)}" placeholder="للإرشاد فقط">
            </div>
            <div class="field" style="min-width:220px">
              <label>ملاحظة</label>
              <input value="${m==="manual"?"يدوي":"تلقائي"}" disabled>
    ...
    // Update fields (IMPORTANT: updates the real object)
...
btnLoadActive.onclick = async ()=>{
  const qy = query(collection(db,"exams"), where("status","==","active"));
  const snap = await getDocs(qy);
  if (snap.empty) return alert("لا يوجد امتحان Active");
...
btnSave.onclick = async ()=>{
  ...
  if (examData.status === "active"){
    await deactivateOtherActiveExams(examId);
  }
